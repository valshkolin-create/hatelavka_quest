...
/* ==========================================
   🧩 ЛОГИКА TELEGRAM ИСПЫТАНИЙ (Twitch-Style)
   ========================================== */

// Функция обновления статуса (Подписки, Голоса, Прогресс)
window.updateTelegramStatus = async function() {
    console.log("🔄 Обновляем Telegram задания...");
    try {
        ...
        if (countEl) countEl.innerText = rCount;
...
// Проверка профиля (Фамилия и Био)
window.checkTelegramProfile = async function() {
    ...
    try {
        ...
        // Обработка фамилии
        if (btn1) {
            if (data.surname) markTgAsDone(btn1);
            else {
        ...
        // Обновляем общий статус (чтобы перерисовались билеты или другие элементы)
...
// Хелпер: Красит кнопку в "Выполнено"
...
// Запускаем проверку через 1.5 секунды после загрузки скрипта
setTimeout(window.updateTelegramStatus, 1500);
