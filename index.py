...
async def sleep_mode_check(request: Request, call_next):
    ...
    now = time.time()
    if (now - sleep_cache["last_checked"]) > 10: 
        try:
...
async def handle_quest_reaction(update: Update):
    ...
    try:
        # Используем фоновый клиент для скорости
        client = await get_background_client()
        ...
        if now - last_reset > timedelta(days=7):
            ...
            await client.patch(
                ...
                json={"last_reaction_reset": now.isoformat(), "reaction_count_weekly": 0}
            )
        ...
        if count < TG_REACTION_WEEKLY_LIMIT:
            ...
            await client.patch(
                "/telegram_challenges", 
                params={"user_id": f"eq.{user_id}"},
                json={"reaction_count_weekly": new_count}
            )
            # Билет
...
async def get_telegram_status(
    request: Request,
    supabase: httpx.AsyncClient = Depends(get_supabase_client) # Используем быстрый клиент
):
    ...
    try:
        ...
        # 1. Запрашиваем состояние из базы (HTTPX стиль)
        res = await supabase.get("/telegram_challenges", params={"user_id": f"eq.{user_id}"})
        ...
        if not res.json():
            ...
            await supabase.post("/telegram_challenges", json=record)
        else:
            record = res.json()[0]
        ...
        if not is_sub and TG_QUEST_CHANNEL_ID != 0:
            try:
                ...
                if member.status in ["member", "administrator", "creator", "restricted"]:
                    is_sub = True
                    # Фиксируем в базе
                    await supabase.patch("/telegram_challenges", params={"user_id": f"eq.{user_id}"}, json={"is_subscribed": True})
                    # Начисляем 5 билетов через RPC
                    await supabase.post("/rpc/increment_tickets", json={"p_user_id": user_id, "p_amount": 5})
            except Exception as e:
                # Ошибку логируем, но не ломаем ответ фронтенду
...
async def check_telegram_profile(
    ...
    try:
        ...
        db_res = await supabase.get("/telegram_challenges", params={"user_id": f"eq.{user_id}"})
        ...
        if s_ok and not curr.get('has_bot_surname'):
            ...
            await supabase.post("/rpc/increment_tickets", json={"p_user_id": user_id, "p_amount": 15})
            
        if b_ok and not curr.get('has_ref_link'):
            updates['has_ref_link'] = True
            await supabase.post("/rpc/increment_tickets", json={"p_user_id": user_id, "p_amount": 20})
            
        if updates:
            await supabase.patch("/telegram_challenges", params={"user_id": f"eq.{user_id}"}, json=updates)
...
async def telegram_vote(
    ...
    try:
        ...
        # Награда
        await supabase.post("/rpc/increment_tickets", json={"p_user_id": user_id, "p_amount": 10})
