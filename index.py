        ...
        try:
            ...
            if roulette_definitions:
                ...
                if in_stock_prizes:
                    ...
                    if user_id: # (Используем 'user_id' вместо 'telegram_id')
                        try:
                            ...
                            await supabase.post(
                                ...
                                json={
                                    "p_user_id": user_id, # (Используем 'user_id')
...
async def issue_twitch_reward_promocode(
    ...
    try:
        ...
        # --- 🔽🔽🔽 ВСТАВЬ ЭТОТ БЛОК 🔽🔽🔽 ---
        reward_settings = reward_data[0]
        reward_type = reward_settings.get("reward_type", "promocode")
        # Используем новое поле amount, если оно есть, иначе старое
        reward_amount = reward_settings.get("reward_amount")
        if reward_amount is None:
        reward_amount = reward_settings.get("promocode_amount", 10)
        # --- 🔼🔼🔼 КОНЕЦ БЛОКА 🔼🔼🔼 ---
...
# --- 🔽🔽🔽 ВСТАВЬ ЭТОТ БЛОК (if / else) 🔽🔽🔽 ---
    if reward_type == "tickets":
        # --- ЛОГИКА ВЫДАЧИ БИЛЕТОВ ---
        logging.info(f"ADMIN: Выдача БИЛЕТОВ ({reward_amount}) для покупки {purchase_id}")

        # 1. Вызываем RPC для начисления билетов
        await supabase.post("/rpc/increment_tickets", json={"p_user_id": user_id, "p_amount": reward_amount})

        # 2. Помечаем покупку как выданную
        await supabase.patch(
            "/twitch_reward_purchases",
            params={"id": f"eq.{purchase_id}"},
        ...
        notification_text = (
            f"<b>🎟️ Ваша награда за «{html_decoration.quote(reward_settings.get('title'))}»!</b>\n\n"
            f"Администратор начислил вам <b>{reward_amount} билетов</b>."
        )
        background_tasks.add_task(safe_send_message, user_id, text=notification_text)

        return {"message": f"Билеты ({reward_amount}) успешно выданы пользователю."}

    else:
        # --- ЛОГИКА ВЫДАЧИ ПРОМОКОДА (старый код) ---
        logging.info(f"ADMIN: Выдача ПРОМОКОДА для покупки {purchase_id}")
    # --- 🔼🔼🔼 КОНЕЦ БЛОКА 🔼🔼🔼
